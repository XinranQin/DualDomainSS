...
if start_epoch > 0:
    ...
    model.load_state_dict(torch.load('%s/net_withnonoise_params_%d.pkl' % (pre_model_dir, start_epoch)))
...
for epoch_i in range(start_epoch, end_epoch + 1):
    ...
    if epoch_i % 10 == 0:
        ...
        torch.save(model.state_dict(),
                   "./%s/net_withnonoise_params_%d.pkl" % (model_dir, epoch_i))  # save only the parameters
        ...
        output_data = "CS ratio is %d, Avg PSNR/SSIM for %s is %.2f/%.4f, Epoch number of model is %d \n" % (
            cs_ratio, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), epoch_i)
        print(output_data)
